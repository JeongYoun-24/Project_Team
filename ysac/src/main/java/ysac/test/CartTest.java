...
public class CartTest {
	...
	List<CartJoinVO> cart = cartSqlMapper.getCartById2(user_id);
	System.out.println("값 나와라 "+cart);
	List<CartJoinDTO> cartDTOList = new ArrayList<>();
	
	for(CartJoinVO cart2: cart){
//		CartDTO movieImgDTO = CartDTO.of(cart2);// entity->dto 메서드호출
		CartJoinDTO dto = CartJoinDTO.builder()
				.cart(1)
