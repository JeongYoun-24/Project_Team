...
public interface CartSqlMapper {
    ...
    @Delete("DELETE FROM CART WHERE PRODUCT_CODE = #{product_code} AND USER_ID = #{user_id} AND cart = #{cart}")
    public int deleteCart(@Param("cart") int cart,@Param("user_id") String user_id,@Param("product_code")String product_code);

    @Delete("DELETE FROM CART WHERE cart = #{cart}")
    public int deleteCart2(@Param("cart") int cart);
