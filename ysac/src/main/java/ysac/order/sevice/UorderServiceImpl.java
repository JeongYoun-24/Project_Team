...
public class UorderServiceImpl implements UorderService{
    ...
    public UorderServiceImpl() {
        ...
        uorderSqlMapper = session.getMapper(UorderSqlMapper.class);
    ...
    @Override
    public void updateOrder(int order_status,int order_code,String user_id) {
//        UorderVO vo = modelMapper.map(dto, UorderVO.class);
        uorderSqlMapper.updateOrder(order_status,order_code,user_id);
        session.commit();
    }
	...
	public int ordercount(String user_id) {
		int orderCount = uorderSqlMapper.orderCount(user_id);
		return orderCount;
