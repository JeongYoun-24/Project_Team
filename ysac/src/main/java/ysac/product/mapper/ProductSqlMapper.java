...
public interface ProductSqlMapper {
	...
	String allList = """
			select * from (
		    select rownum as recNum, product_code, category, pro_name, pro_info,pro_content, pro_price, pro_img
		    from (
		        select product_code, category, pro_name, pro_info,pro_content, pro_price, pro_img from product
	...
	@Select(nameList)
	public List<ProductVO> proNameSelectList(Map<String, String> pageingMap);
	...
	@Select(" select count(product_code) from product where pro_name LIKE '%' || #{pro_name} || '%' ")
	public int proNameTotNum(@Param("pro_name") String pro_name);
	
	@Select("select max(product_code)+1 from product") // 리뷰 글 번호 생성
